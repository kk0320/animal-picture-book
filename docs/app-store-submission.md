...
生きものごとの写真、すんでいるところ、食べもの、特徴、まめちしきを収録しています。一覧から探したり、お気に入りに入れたり、前へ・次へ・ランダム表示でいろいろな生きものを見つけられます。詳細画面では、説明文をひらがな中心で読めるひらがなモードに切り替えられます。
...
このアプリは、100種類のどうぶつ・むし・海の生きものの写真と説明文をアプリ内に同梱したオフライン図鑑です。ログイン、広告、アプリ内課金、解析、外部通信SDKは使用していません。主要機能は一覧、検索、お気に入り、普通モード / ひらがなモード切り替え、前後移動、ランダム表示、スワイプ操作です。インターネット接続なしでも閲覧できます。
...
- 広告データ、解析データ: なし
- トラッキング: なし
- お気に入り・表示モード設定: 端末内保存のみ。外部送信なし。

表示モード設定は `animal-picture-book-reading-mode` として端末内に保存される。普通モードとひらがなモードの切り替え状態のみで、外部サーバーへ送信しない。
...
- 初回起動、検索、お気に入り、ひらがなモード切り替え、詳細遷移が動く
- 機内モード確認が通る
