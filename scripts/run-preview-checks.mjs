...
const baseUrl = process.env.APP_URL ?? "http://127.0.0.1:4183";
...
try {
  await waitForServer();
  await runNodeScript("browser-check.mjs");
  await runNodeScript("offline-check.mjs");
} finally {
  server.kill();
  await fs.writeFile(path.join(reportDir, "preview-check.out.txt"), serverLog.join(""), "utf8");
}
